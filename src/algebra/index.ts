...
export namespace powerflow {
  export class PowerBusResult extends $sdk.GCObject {
    ...
    static createFrom({abs, angle_radians, voltage, voltage_img, current, current_img}: {abs: number, angle_radians: number, voltage: number, voltage_img: number, current: number, current_img: number}, $g: $sdk.GreyCat = globalThis.greycat.default): PowerBusResult {
      return new PowerBusResult($g.abi.libs_by_name.get(algebralib.name)!.mapped[173], abs, angle_radians, voltage, voltage_img, current, current_img);
    }
...
export const algebralib: $sdk.Library = {
  ...
  init(abi) {
    ...
    this.mapped[166] = abi.type_by_fqn.get(ml.Polynomial._type);
    this.mapped[167] = abi.type_by_fqn.get(ml.GaussianND._type);
    this.mapped[168] = abi.type_by_fqn.get(ml.Solver._type);
    this.mapped[169] = abi.type_by_fqn.get(ml.TimeSeriesDecomposition._type);
    this.mapped[170] = abi.type_by_fqn.get(ml.PCA._type);
    if (this.mapped[170]) {
      this.mapped[170].static_values = {['threshold_def']: 0.95,};
    }
    this.mapped[171] = abi.type_by_fqn.get(ml.HeatMapProfile._type);
    this.mapped[172] = abi.type_by_fqn.get(transforms.FFTModel._type);
    this.mapped[173] = abi.type_by_fqn.get(powerflow.PowerBusResult._type);
