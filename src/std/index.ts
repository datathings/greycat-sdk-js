...
export namespace util {
  ...
  export class Stack<T = any> extends $greycat.GCObject {
    ...
    static createFrom<T>({values}: {values: globalThis.Array<any | null> | null}, $g: $greycat.GreyCat = $greycat.$.default): Stack {
      return new Stack($g.abi.libs_by_name.get(stdlib.name)!.mapped[120], values);
    }
    static create<T>(values: globalThis.Array<any | null> | null, $g: $greycat.GreyCat = $greycat.$.default): Stack<T> {
      return new Stack($g.abi.libs_by_name.get(stdlib.name)!.mapped[120], values);
    }
  }
