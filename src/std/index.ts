...
export const stdlib: Library = {
  ...
  configure(loaders, factories) {
    ...
    loaders.set(io.JsonReader._type, std_n.io.JsonReader.load);
    factories.set(io.GcbWriter._type, io.GcbWriter);
    loaders.set(io.GcbWriter._type, std_n.io.GcbWriter.load);
    ...
    loaders.set(io.FileWalker._type, std_n.io.FileWalker.load);
  ...
  init(abi) {
    ...
    this.mapped[14] = abi.type_by_fqn.get(core.DurationUnit._type)!;
    this.mapped[14].resolveGeneratedOffsetWithValues('microseconds', 1,'milliseconds', 1000,'seconds', 1000000,'minutes', 60000000,'hours', 3600000000,'days', 86400000000,'weeks', 604800000000,'months', 2628000000000,'years', 31536000000000);
    this.mapped[15] = abi.type_by_fqn.get(core.Table._type)!;
    this.mapped[16] = abi.type_by_fqn.get(core.DatePart._type)!;
    this.mapped[16].resolveGeneratedOffsetWithValues('years', 0,'months', 1,'days', 2,'hours', 3,'minutes', 4,'seconds', 5,'microseconds', 6);
    this.mapped[17] = abi.type_by_fqn.get(core.GeoCircle._type)!;
    this.mapped[17].resolveGeneratedOffsets('center','radius');
    this.mapped[18] = abi.type_by_fqn.get(core.Error._type)!;
    this.mapped[19] = abi.type_by_fqn.get(core.Map._type)!;
    this.mapped[20] = abi.type_by_fqn.get(core.GeoBox._type)!;
    this.mapped[20].resolveGeneratedOffsets('sw','ne');
    this.mapped[21] = abi.type_by_fqn.get(core.Date._type)!;
    this.mapped[22] = abi.type_by_fqn.get(core.ti3d._type)!;
    this.mapped[23] = abi.type_by_fqn.get(core.ti10d._type)!;
    this.mapped[24] = abi.type_by_fqn.get(core.ti5d._type)!;
    this.mapped[25] = abi.type_by_fqn.get(core.duration._type)!;
    this.mapped[26] = abi.type_by_fqn.get(core.nodeTime._type)!;
    this.mapped[27] = abi.type_by_fqn.get(core.Array._type)!;
    this.mapped[28] = abi.type_by_fqn.get(core.function_._type)!;
    this.mapped[29] = abi.type_by_fqn.get(core.NodeTimeInfo._type)!;
    ...
    this.mapped[33].static_values = [abi.createTime(-9223372036854775808n),abi.createTime(9223372036854775807n)];
    this.mapped[34] = abi.type_by_fqn.get(core.ti6d._type)!;
    this.mapped[35] = abi.type_by_fqn.get(core.Tensor._type)!;
    ...
    this.mapped[38] = abi.type_by_fqn.get(core.ti2d._type)!;
    ...
    this.mapped[41] = abi.type_by_fqn.get(runtime.UserGroupPolicyType._type)!;
    this.mapped[41].resolveGeneratedOffsetWithValues('read', 0,'write', 1,'execute', 2);
    this.mapped[42] = abi.type_by_fqn.get(runtime.Task._type)!;
    this.mapped[42].resolveGeneratedOffsets('id','user','parent','mod','type','fun','creation','status','start','progress','remaining','duration','sub_waiting','sub_tasks_all');
    this.mapped[43] = abi.type_by_fqn.get(runtime.User._type)!;
    this.mapped[43].resolveGeneratedOffsets('id','name','activated','full_name','email','role','permissions_flags','groups','groups_flags','external');
    this.mapped[44] = abi.type_by_fqn.get(runtime.System._type)!;
    this.mapped[45] = abi.type_by_fqn.get(runtime.UserGroupPolicy._type)!;
    this.mapped[45].resolveGeneratedOffsets('group_id','type');
    ...
    this.mapped[50].resolveGeneratedOffsets('capacity_bytes','allocated_bytes','allocated_ratio','remained_bytes','remained_ratio','used_bytes','used_ratio','available_bytes','available_ratio');
    ...
    this.mapped[54].resolveGeneratedOffsets('name','permissions');
    this.mapped[55] = abi.type_by_fqn.get(runtime.RuntimeInfo._type)!;
    this.mapped[55].resolveGeneratedOffsets('version','arch','timezone','license','io_threads','bg_threads','fg_threads','mem_total','mem_worker','nb_ctx','store_stats');
    this.mapped[56] = abi.type_by_fqn.get(runtime.License._type)!;
    this.mapped[56].resolveGeneratedOffsets('name','start','end','company','max_workers','max_memory','max_store','extra_1','extra_2','extra_3','extra_4');
    this.mapped[57] = abi.type_by_fqn.get(io.File._type)!;
    this.mapped[57].resolveGeneratedOffsets('path','size','last_modification');
    this.mapped[58] = abi.type_by_fqn.get(io.JsonReader._type)!;
    this.mapped[59] = abi.type_by_fqn.get(io.GcbWriter._type)!;
    this.mapped[60] = abi.type_by_fqn.get(io.CsvColumnInteger._type)!;
    ...
    this.mapped[62].resolveGeneratedOffsets('protocol','host','port','path','params','hash');
    this.mapped[63] = abi.type_by_fqn.get(io.GcbReader._type)!;
    ...
    this.mapped[65] = abi.type_by_fqn.get(io.CsvReader._type)!;
    this.mapped[66] = abi.type_by_fqn.get(io.HttpHeader._type)!;
    this.mapped[66].resolveGeneratedOffsets('name','value');
    this.mapped[67] = abi.type_by_fqn.get(io.CsvWriter._type)!;
    this.mapped[68] = abi.type_by_fqn.get(io.TextWriter._type)!;
    ...
    this.mapped[70] = abi.type_by_fqn.get(io.JsonWriter._type)!;
    this.mapped[71] = abi.type_by_fqn.get(io.TextReader._type)!;
    ...
    this.mapped[73] = abi.type_by_fqn.get(io.CsvColumn._type)!;
    this.mapped[73].resolveGeneratedOffsets('name','mandatory');
    this.mapped[74] = abi.type_by_fqn.get(io.Email._type)!;
    this.mapped[74].resolveGeneratedOffsets('from','subject','body','body_is_html','to','cc','bcc');
    this.mapped[75] = abi.type_by_fqn.get(io.SmtpMode._type)!;
    this.mapped[75].resolveGeneratedOffsetWithValues('plain', 0,'ssl_tls', 1,'starttls', 2);
    this.mapped[76] = abi.type_by_fqn.get(io.CsvColumnFloat._type)!;
    ...
    this.mapped[78].resolveGeneratedOffsets('name','mandatory','format','tz','as_time');
    this.mapped[79] = abi.type_by_fqn.get(io.CsvColumnTime._type)!;
    this.mapped[79].resolveGeneratedOffsets('name','mandatory','unit');
    this.mapped[80] = abi.type_by_fqn.get(io.Http._type)!;
    this.mapped[81] = abi.type_by_fqn.get(io.FileWalker._type)!;
    this.mapped[82] = abi.type_by_fqn.get(io.TextEncoder._type)!;
    this.mapped[82].resolveGeneratedOffsetWithValues('plain', null,'base64', null,'base64url', null,'hexadecimal', null);
    ...
    this.mapped[84].static_values = [2.7182818285,1.4426950409,0.4342944819,0.6931471806,2.302585093,3.1415926536,1.5707963268,0.7853981634,0.3183098862,0.6366197724,1.1283791671,1.4142135624,0.7071067812];
    this.mapped[85] = abi.type_by_fqn.get(util.BoxPlotI64._type)!;
    this.mapped[85].resolveGeneratedOffsets('min','max','whiskerLow','whiskerHigh','percentile1','percentile5','percentile25','percentile50','percentile75','percentile95','percentile99','countOutliersLow','countOutliersHigh','percentageOutliersLow','percentageOutliersHigh','sum','avg','std','size');
    ...
    this.mapped[89].resolveGeneratedOffsets('seed','v');
    this.mapped[90] = abi.type_by_fqn.get(util.HistogramBucket._type)!;
    this.mapped[90].resolveGeneratedOffsetWithValues('from', 0,'to', 1,'count', 2,'percentage', 3);
    this.mapped[91] = abi.type_by_fqn.get(util.Crypto._type)!;
    this.mapped[92] = abi.type_by_fqn.get(util.Queue._type)!;
    this.mapped[93] = abi.type_by_fqn.get(util.HistogramF64._type)!;
    this.mapped[94] = abi.type_by_fqn.get(util.ProgressTracker._type)!;
    this.mapped[95] = abi.type_by_fqn.get(util.Quantizer._type)!;
    this.mapped[96] = abi.type_by_fqn.get(util.Histogram._type)!;
    this.mapped[97] = abi.type_by_fqn.get(util.Iban._type)!;
    this.mapped[98] = abi.type_by_fqn.get(util.TimeWindow._type)!;
    this.mapped[99] = abi.type_by_fqn.get(util.Gaussian._type)!;
    this.mapped[100] = abi.type_by_fqn.get(util.Buffer._type)!;
    this.mapped[101] = abi.type_by_fqn.get(util.BoxPlotF64._type)!;
    ...
    this.mapped[102] = abi.type_by_fqn.get(util.HistogramI64._type)!;
    this.mapped[103] = abi.type_by_fqn.get(util.BoxPlot._type)!;
    this.mapped[103].resolveGeneratedOffsets('min','max','whiskerLow','whiskerHigh','percentile1','percentile5','percentile25','percentile50','percentile75','percentile95','percentile99','countOutliersLow','countOutliersHigh','percentageOutliersLow','percentageOutliersHigh','sum','avg','std','size');
