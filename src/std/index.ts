...
export const stdlib: Library = {
  ...
  init(abi) {
    ...
    this.mapped[4] = abi.type_by_fqn.get(core.ti3d._type)!;
    this.mapped[5] = abi.type_by_fqn.get(core.ti2d._type)!;
    this.mapped[6] = abi.type_by_fqn.get(core.node._type)!;
    this.mapped[7] = abi.type_by_fqn.get(core.tf2d._type)!;
    this.mapped[8] = abi.type_by_fqn.get(core.nodeTimeCursor._type)!;
    this.mapped[9] = abi.type_by_fqn.get(core.TableColumnMeta._type)!;
    this.mapped[9].resolveGeneratedOffsets('type','size','index','min','max','avg','std');
    this.mapped[10] = abi.type_by_fqn.get(core.Error._type)!;
    this.mapped[11] = abi.type_by_fqn.get(core.Map._type)!;
    ...
    this.mapped[15] = abi.type_by_fqn.get(core.SamplingMode._type)!;
    this.mapped[15].resolveGeneratedOffsetWithValues('fixed', 0,'fixed_reg', 1,'adaptative', 2,'dense', 3);
    ...
    this.mapped[18] = abi.type_by_fqn.get(core.Table._type)!;
    this.mapped[19] = abi.type_by_fqn.get(core.Date._type)!;
    this.mapped[20] = abi.type_by_fqn.get(core.ti5d._type)!;
    this.mapped[21] = abi.type_by_fqn.get(core.ti6d._type)!;
    this.mapped[22] = abi.type_by_fqn.get(core.duration._type)!;
    this.mapped[23] = abi.type_by_fqn.get(core.nodeTime._type)!;
    this.mapped[24] = abi.type_by_fqn.get(core.Array._type)!;
    this.mapped[25] = abi.type_by_fqn.get(core.DatePart._type)!;
    this.mapped[25].resolveGeneratedOffsetWithValues('years', 0,'months', 1,'days', 2,'hours', 3,'minutes', 4,'seconds', 5,'microseconds', 6);
    this.mapped[26] = abi.type_by_fqn.get(core.function_._type)!;
    this.mapped[27] = abi.type_by_fqn.get(core.NodeTimeInfo._type)!;
    this.mapped[27].resolveGeneratedOffsets('size','from','to');
    ...
    this.mapped[30].resolveGeneratedOffsets('center','radius');
    ...
    this.mapped[34] = abi.type_by_fqn.get(core.tf4d._type)!;
    ...
    this.mapped[36].static_values = [abi.createTime(-9223372036854775808n),abi.createTime(9223372036854775807n)];
    this.mapped[37] = abi.type_by_fqn.get(core.geo._type)!;
    this.mapped[37].static_values = [abi.createGeo(-85.0511287602,-179.9999999581),abi.createGeo(85.0511287602,179.9999999581)];
    this.mapped[38] = abi.type_by_fqn.get(core.String._type)!;
    this.mapped[39] = abi.type_by_fqn.get(runtime.Task._type)!;
    this.mapped[39].resolveGeneratedOffsets('id','user','parent','mod','type','fun','creation','status','start','progress','remaining','duration','sub_waiting','sub_tasks_all');
    this.mapped[40] = abi.type_by_fqn.get(runtime.SecurityEntity._type)!;
    this.mapped[40].resolveGeneratedOffsets('id','name','activated');
    this.mapped[41] = abi.type_by_fqn.get(runtime.TaskStatus._type)!;
    this.mapped[41].resolveGeneratedOffsetWithValues('empty', 0,'waiting', 1,'running', 2,'cancelled', 3,'error', 4,'ended', 5);
    ...
    this.mapped[44].resolveGeneratedOffsets('id','name','activated');
    ...
    this.mapped[46].resolveGeneratedOffsets('name','permissions');
    ...
    this.mapped[50] = abi.type_by_fqn.get(runtime.RuntimeInfo._type)!;
    this.mapped[50].resolveGeneratedOffsets('version','arch','timezone','license','io_threads','bg_threads','fg_threads','mem_total','mem_worker','nb_ctx','store_stats');
    ...
    this.mapped[52].resolveGeneratedOffsets('name','start','end','company','max_workers','max_memory','max_store','extra_1','extra_2','extra_3','extra_4');
    this.mapped[53] = abi.type_by_fqn.get(runtime.UserGroupPolicyType._type)!;
    this.mapped[53].resolveGeneratedOffsetWithValues('read', 0,'write', 1,'execute', 2);
    this.mapped[54] = abi.type_by_fqn.get(runtime.UserCredential._type)!;
    this.mapped[54].resolveGeneratedOffsets('offset','pass');
    this.mapped[55] = abi.type_by_fqn.get(runtime.StoreStat._type)!;
    this.mapped[55].resolveGeneratedOffsets('capacity_bytes','allocated_bytes','allocated_ratio','remained_bytes','remained_ratio','used_bytes','used_ratio','available_bytes','available_ratio');
    ...
    this.mapped[60] = abi.type_by_fqn.get(io.GcbReader._type)!;
    this.mapped[61] = abi.type_by_fqn.get(io.CsvColumn._type)!;
    this.mapped[61].resolveGeneratedOffsets('name','mandatory');
    this.mapped[62] = abi.type_by_fqn.get(io.JsonReader._type)!;
    this.mapped[63] = abi.type_by_fqn.get(io.JsonWriter._type)!;
    this.mapped[64] = abi.type_by_fqn.get(io.FileEntry._type)!;
    this.mapped[65] = abi.type_by_fqn.get(io.File._type)!;
    this.mapped[66] = abi.type_by_fqn.get(io.Directory._type)!;
    this.mapped[67] = abi.type_by_fqn.get(io.GcbWriter._type)!;
    this.mapped[68] = abi.type_by_fqn.get(io.FileDescriptor._type)!;
    this.mapped[68].resolveGeneratedOffsets('uri','size','last_modification');
    this.mapped[69] = abi.type_by_fqn.get(io.CsvWriter._type)!;
    this.mapped[70] = abi.type_by_fqn.get(io.TextWriter._type)!;
    this.mapped[71] = abi.type_by_fqn.get(io.CsvColumnInteger._type)!;
    this.mapped[71].resolveGeneratedOffsets('name','mandatory');
    this.mapped[72] = abi.type_by_fqn.get(io.CsvColumnTime._type)!;
    this.mapped[72].resolveGeneratedOffsets('name','mandatory','unit');
    this.mapped[73] = abi.type_by_fqn.get(io.JSON._type)!;
    this.mapped[74] = abi.type_by_fqn.get(io.CsvReader._type)!;
    this.mapped[75] = abi.type_by_fqn.get(io.TextReader._type)!;
    ...
    this.mapped[77] = abi.type_by_fqn.get(io.CsvFormat._type)!;
    this.mapped[77].resolveGeneratedOffsets('header_lines','infer','separator','string_delimiter','decimal_separator','thousands_separator','columns');
    this.mapped[78] = abi.type_by_fqn.get(io.CsvColumnIgnored._type)!;
    this.mapped[78].resolveGeneratedOffsets('name','mandatory');
    this.mapped[79] = abi.type_by_fqn.get(io.CsvColumnFloat._type)!;
    this.mapped[79].resolveGeneratedOffsets('name','mandatory');
    this.mapped[80] = abi.type_by_fqn.get(debug.TypeRef._type)!;
    this.mapped[80].resolveGeneratedOffsets('name','module','generics','nullable');
    this.mapped[81] = abi.type_by_fqn.get(debug.TypeDesc._type)!;
    this.mapped[81].resolveGeneratedOffsets('module','name','methods','attrs','is_abstract','is_enum');
    this.mapped[82] = abi.type_by_fqn.get(debug.FnDesc._type)!;
    this.mapped[82].resolveGeneratedOffsets('module','name','parent_type','params','return_type','is_static','is_task','is_exclusive','is_reserved','is_exposed','mode','permissions');
    this.mapped[83] = abi.type_by_fqn.get(debug.FnParam._type)!;
    this.mapped[83].resolveGeneratedOffsets('name','type');
    this.mapped[84] = abi.type_by_fqn.get(debug.ModDesc._type)!;
    this.mapped[84].resolveGeneratedOffsets('name','lib','functions','types','vars');
    this.mapped[85] = abi.type_by_fqn.get(debug.TypeAttr._type)!;
    this.mapped[85].resolveGeneratedOffsets('name','type','is_static');
    this.mapped[86] = abi.type_by_fqn.get(debug.FnMode._type)!;
    this.mapped[86].resolveGeneratedOffsetWithValues('read_only', null,'copy_on_write', null,'volatile', null,'write', null);
    this.mapped[87] = abi.type_by_fqn.get(debug.ModVarDesc._type)!;
    this.mapped[87].resolveGeneratedOffsets('module','name','type');
    this.mapped[88] = abi.type_by_fqn.get(math.MathConstants._type)!;
    this.mapped[88].static_values = [2.7182818285,1.4426950409,0.4342944819,0.6931471806,2.302585093,3.1415926536,1.5707963268,0.7853981634,0.3183098862,0.6366197724,1.1283791671,1.4142135624,0.7071067812];
    ...
    this.mapped[92] = abi.type_by_fqn.get(util.Buffer._type)!;
    this.mapped[93] = abi.type_by_fqn.get(util.BoxPlot._type)!;
    ...
    this.mapped[95] = abi.type_by_fqn.get(util.Queue._type)!;
    this.mapped[96] = abi.type_by_fqn.get(util.GaussianProfile._type)!;
    this.mapped[97] = abi.type_by_fqn.get(util.NdEncoding._type)!;
    this.mapped[98] = abi.type_by_fqn.get(util.BoxPlotI64._type)!;
    ...
    this.mapped[99] = abi.type_by_fqn.get(util.Random._type)!;
    this.mapped[99].resolveGeneratedOffsets('seed','v');
    this.mapped[100] = abi.type_by_fqn.get(util.Gaussian._type)!;
    this.mapped[101] = abi.type_by_fqn.get(util.Assert._type)!;
    this.mapped[102] = abi.type_by_fqn.get(util.Histogram._type)!;
    this.mapped[103] = abi.type_by_fqn.get(util.HistogramF64._type)!;
    this.mapped[104] = abi.type_by_fqn.get(util.HistogramBucket._type)!;
    this.mapped[104].resolveGeneratedOffsetWithValues('from', 0,'to', 1,'count', 2,'percentage', 3);
    this.mapped[105] = abi.type_by_fqn.get(util.BoxPlotF64._type)!;
    ...
    this.mapped[106] = abi.type_by_fqn.get(util.Quantizer._type)!;
    this.mapped[107] = abi.type_by_fqn.get(util.HistogramI64._type)!;
    this.mapped[108] = abi.type_by_fqn.get(util.ByteArray._type)!;
    this.mapped[109] = abi.type_by_fqn.get(util.ProgressTracker._type)!;
  },
};
