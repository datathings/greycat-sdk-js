...
export const stdlib: Library = {
  ...
  init(abi) {
    ...
    this.mapped[58] = abi.type_by_fqn.get(io.TextFormat._type)!;
    this.mapped[59] = abi.type_by_fqn.get(io.AbiReader._type)!;
    this.mapped[60] = abi.type_by_fqn.get(io.BinaryFormat._type)!;
    this.mapped[61] = abi.type_by_fqn.get(io.FileDescriptor._type)!;
    this.mapped[61].resolveGeneratedOffsets('uri','size','last_modification');
    this.mapped[62] = abi.type_by_fqn.get(io.JsonWriter._type)!;
    ...
    this.mapped[65] = abi.type_by_fqn.get(io.CSVColumnString._type)!;
    this.mapped[65].resolveGeneratedOffsets('name','mandatory','trim','try_number','try_json','values','encoder');
    this.mapped[66] = abi.type_by_fqn.get(io.CSVColumnBoolean._type)!;
    this.mapped[66].resolveGeneratedOffsets('name','mandatory');
    this.mapped[67] = abi.type_by_fqn.get(io.TextWriter._type)!;
    this.mapped[68] = abi.type_by_fqn.get(io.CSVColumn._type)!;
    this.mapped[68].resolveGeneratedOffsets('name','mandatory');
    this.mapped[69] = abi.type_by_fqn.get(io.FileWriter._type)!;
    this.mapped[70] = abi.type_by_fqn.get(io.CSVColumnInteger._type)!;
    this.mapped[70].resolveGeneratedOffsets('name','mandatory');
    this.mapped[71] = abi.type_by_fqn.get(io.Directory._type)!;
    this.mapped[72] = abi.type_by_fqn.get(io.CSVColumnIgnored._type)!;
    this.mapped[72].resolveGeneratedOffsets('name','mandatory');
    this.mapped[73] = abi.type_by_fqn.get(io.FileEntry._type)!;
    ...
    this.mapped[75] = abi.type_by_fqn.get(io.JsonFormat._type)!;
    this.mapped[76] = abi.type_by_fqn.get(io.TextReader._type)!;
    this.mapped[77] = abi.type_by_fqn.get(io.FileFormat._type)!;
    this.mapped[78] = abi.type_by_fqn.get(io.CSVColumnFloat._type)!;
    ...
    this.mapped[81].resolveGeneratedOffsets('name','mandatory','format','tz','as_time');
    this.mapped[82] = abi.type_by_fqn.get(debug.ModVarDesc._type)!;
    this.mapped[82].resolveGeneratedOffsets('module','name','type');
    this.mapped[83] = abi.type_by_fqn.get(debug.TypeAttr._type)!;
    this.mapped[83].resolveGeneratedOffsets('name','type','is_static');
    ...
    this.mapped[85] = abi.type_by_fqn.get(debug.FnParam._type)!;
    ...
    this.mapped[88] = abi.type_by_fqn.get(debug.TypeRef._type)!;
    this.mapped[88].resolveGeneratedOffsets('name','module','generics','nullable');
    this.mapped[89] = abi.type_by_fqn.get(debug.FnDesc._type)!;
    this.mapped[89].resolveGeneratedOffsets('module','name','parent_type','params','return_type','is_static','is_task','is_exclusive','is_reserved','is_exposed','mode','permissions');
    this.mapped[90] = abi.type_by_fqn.get(math.MathConstants._type)!;
    this.mapped[90].static_values = [2.7182818285,1.4426950409,0.4342944819,0.6931471806,2.302585093,3.1415926536,1.5707963268,0.7853981634,0.3183098862,0.6366197724,1.1283791671,1.4142135624,0.7071067812];
    this.mapped[91] = abi.type_by_fqn.get(util.Iban._type)!;
    this.mapped[92] = abi.type_by_fqn.get(util.BoxPlot._type)!;
    ...
    this.mapped[95] = abi.type_by_fqn.get(util.GaussianProfile._type)!;
    this.mapped[96] = abi.type_by_fqn.get(util.HistogramF64._type)!;
    this.mapped[97] = abi.type_by_fqn.get(util.Quantizer._type)!;
    this.mapped[98] = abi.type_by_fqn.get(util.HistogramBucket._type)!;
    this.mapped[98].resolveGeneratedOffsetWithValues('from', 0,'to', 1,'count', 2,'percentage', 3);
    this.mapped[99] = abi.type_by_fqn.get(util.Buffer._type)!;
    this.mapped[100] = abi.type_by_fqn.get(util.BoxPlotF64._type)!;
    this.mapped[100].resolveGeneratedOffsets('min','max','whiskerLow','whiskerHigh','percentile1','percentile5','percentile25','percentile50','percentile75','percentile95','percentile99','countOutliersLow','countOutliersHigh','percentageOutliersLow','percentageOutliersHigh','sum','avg','std','size');
    this.mapped[101] = abi.type_by_fqn.get(util.Histogram._type)!;
    this.mapped[102] = abi.type_by_fqn.get(util.Queue._type)!;
    this.mapped[103] = abi.type_by_fqn.get(util.Crypto._type)!;
    this.mapped[104] = abi.type_by_fqn.get(util.ByteArray._type)!;
    this.mapped[105] = abi.type_by_fqn.get(util.BoxPlotI64._type)!;
    ...
    this.mapped[107] = abi.type_by_fqn.get(util.Random._type)!;
    this.mapped[107].resolveGeneratedOffsets('seed','v');
    this.mapped[108] = abi.type_by_fqn.get(util.HistogramI64._type)!;
    this.mapped[109] = abi.type_by_fqn.get(util.SlidingWindow._type)!;
    this.mapped[110] = abi.type_by_fqn.get(util.Gaussian._type)!;
    this.mapped[111] = abi.type_by_fqn.get(util.Assert._type)!;
  },
};
