...
export class function_ extends $sdk.GCObject {
  ...
  static create($g: $sdk.GreyCat): function_ {
    return new function_($g.abi.libs_by_name.get($sdk.std.stdlib.name)!.mapped[12]);
  }
}
...
export class Http extends $sdk.GCObject {
  ...
  static create($g: $sdk.GreyCat): Http {
    return new Http($g.abi.libs_by_name.get($sdk.std.stdlib.name)!.mapped[74]);
  }
}
...
export class Assert extends $sdk.GCObject {
  ...
  static create($g: $sdk.GreyCat): Assert {
    return new Assert($g.abi.libs_by_name.get($sdk.std.stdlib.name)!.mapped[89]);
  }
}
...
export class Crypto extends $sdk.GCObject {
  ...
  static create($g: $sdk.GreyCat): Crypto {
    return new Crypto($g.abi.libs_by_name.get($sdk.std.stdlib.name)!.mapped[91]);
  }
}
...
export const stdlib: $sdk.Library = {
  ...
  init(abi) {
    ...
    this.mapped[15] = abi.type_by_fqn.get(core.ti2d._type)!;
    this.mapped[16] = abi.type_by_fqn.get(core.tf3d._type)!;
    this.mapped[17] = abi.type_by_fqn.get(core.TensorType._type)!;
    this.mapped[17].resolveGeneratedOffsetWithValues('i32', 4,'i64', 8,'f32', 4,'f64', 8,'c64', 8,'c128', 16);
    this.mapped[18] = abi.type_by_fqn.get(core.GeoPoly._type)!;
    ...
    this.mapped[20] = abi.type_by_fqn.get(core.duration._type)!;
    this.mapped[21] = abi.type_by_fqn.get(core.node._type)!;
    this.mapped[22] = abi.type_by_fqn.get(core.Date._type)!;
    this.mapped[23] = abi.type_by_fqn.get(core.tf4d._type)!;
    this.mapped[24] = abi.type_by_fqn.get(core.Tensor._type)!;
    ...
    this.mapped[28] = abi.type_by_fqn.get(core.TimeZone._type)!;
    ...
    this.mapped[58] = abi.type_by_fqn.get(io.CsvColumnString._type)!;
    this.mapped[58].resolveGeneratedOffsets('name','mandatory','trim','try_number','try_json','values','encoder');
    this.mapped[59] = abi.type_by_fqn.get(io.CsvColumnTime._type)!;
    this.mapped[59].resolveGeneratedOffsets('name','mandatory','unit');
    this.mapped[60] = abi.type_by_fqn.get(io.CsvColumnDate._type)!;
    this.mapped[60].resolveGeneratedOffsets('name','mandatory','format','tz','as_time');
    this.mapped[61] = abi.type_by_fqn.get(io.CsvColumnFloat._type)!;
    this.mapped[61].resolveGeneratedOffsets('name','mandatory');
    this.mapped[62] = abi.type_by_fqn.get(io.CsvFormat._type)!;
    this.mapped[62].resolveGeneratedOffsets('header_lines','infer','separator','string_delimiter','decimal_separator','thousands_separator','columns');
    ...
    this.mapped[68] = abi.type_by_fqn.get(io.Smtp._type)!;
    this.mapped[68].resolveGeneratedOffsets('host','port','mode','authenticate','user','pass');
    ...
    this.mapped[76] = abi.type_by_fqn.get(util.Iban._type)!;
    ...
    this.mapped[80] = abi.type_by_fqn.get(util.HistogramInt._type)!;
    ...
    this.mapped[82] = abi.type_by_fqn.get(util.Buffer._type)!;
    this.mapped[83] = abi.type_by_fqn.get(util.Gaussian._type)!;
    this.mapped[83].resolveGeneratedOffsets('sum','sum_sq','count','min','max');
    this.mapped[84] = abi.type_by_fqn.get(util.SlidingWindow._type)!;
    this.mapped[85] = abi.type_by_fqn.get(util.Quantizer._type)!;
    this.mapped[86] = abi.type_by_fqn.get(util.Queue._type)!;
    ...
    this.mapped[88].resolveGeneratedOffsets('seed','v');
    this.mapped[89] = abi.type_by_fqn.get(util.Assert._type)!;
    this.mapped[90] = abi.type_by_fqn.get(util.TimeWindow._type)!;
    this.mapped[91] = abi.type_by_fqn.get(util.Crypto._type)!;
  },
};
