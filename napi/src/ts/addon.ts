...
switch (arch) {
  case 'x64':
    switch (platform) {
      case 'linux':
        if (process.env.GREYCAT_CUDA) {
          _addon = require('./greycat.linux-x64-cuda.node');
        } else {
          _addon = require('./greycat.linux-x64.node');
        }
        break;

      case 'darwin':
        _addon = require('./greycat.mac-x64.node');
        break;
    }
    break;
  ...
  case 'arm64':
    switch (platform) {
      case 'linux':
      case 'android':
        _addon = require('./greycat.linux-arm64.node');
        break;
    }
    break;
  ...
  case 'arm':
    switch (platform) {
      case 'linux':
        _addon = require('./greycat.linux-armv7.node');
        break;
    }
    break;
}
