...
export const stdlib: Library = {
  ...
  init(abi) {
    ...
    this.mapped[39] = abi.type_by_fqn.get(runtime.PeriodicTask._type)!;
    ...
    this.mapped[42] = abi.type_by_fqn.get(runtime.SecurityEntity._type)!;
    this.mapped[42].resolveGeneratedOffsets('id','name','activated');
    this.mapped[43] = abi.type_by_fqn.get(runtime.RuntimeInfo._type)!;
    ...
    this.mapped[44] = abi.type_by_fqn.get(runtime.SecurityPolicy._type)!;
    ...
    this.mapped[56] = abi.type_by_fqn.get(runtime.Task._type)!;
    ...
    this.mapped[57] = abi.type_by_fqn.get(io.FileFormat._type)!;
    ...
    this.mapped[59] = abi.type_by_fqn.get(io.AbiReader._type)!;
    this.mapped[60] = abi.type_by_fqn.get(io.JsonWriter._type)!;
    this.mapped[61] = abi.type_by_fqn.get(io.JsonReader._type)!;
    this.mapped[62] = abi.type_by_fqn.get(io.CsvColumnDate._type)!;
    this.mapped[62].resolveGeneratedOffsets('name','mandatory','format','tz','as_time');
    ...
    this.mapped[65] = abi.type_by_fqn.get(io.TextFormat._type)!;
    this.mapped[66] = abi.type_by_fqn.get(io.JSON._type)!;
    this.mapped[67] = abi.type_by_fqn.get(io.CsvWriter._type)!;
    this.mapped[68] = abi.type_by_fqn.get(io.TextWriter._type)!;
    this.mapped[69] = abi.type_by_fqn.get(io.BinaryFormat._type)!;
    this.mapped[70] = abi.type_by_fqn.get(io.FileEntry._type)!;
    this.mapped[71] = abi.type_by_fqn.get(io.CsvColumn._type)!;
    this.mapped[71].resolveGeneratedOffsets('name','mandatory');
    this.mapped[72] = abi.type_by_fqn.get(io.JsonFormat._type)!;
    this.mapped[73] = abi.type_by_fqn.get(io.CsvColumnTime._type)!;
    ...
    this.mapped[76] = abi.type_by_fqn.get(io.Directory._type)!;
    this.mapped[77] = abi.type_by_fqn.get(io.File._type)!;
    this.mapped[78] = abi.type_by_fqn.get(io.TextReader._type)!;
    this.mapped[79] = abi.type_by_fqn.get(io.CsvFormat._type)!;
    this.mapped[79].resolveGeneratedOffsets('header_lines','infer','separator','string_delimiter','decimal_separator','thousands_separator','columns');
    ...
    this.mapped[83] = abi.type_by_fqn.get(debug.ModDesc._type)!;
    this.mapped[83].resolveGeneratedOffsets('name','lib','functions','types','vars');
    this.mapped[84] = abi.type_by_fqn.get(debug.TypeRef._type)!;
    this.mapped[84].resolveGeneratedOffsets('name','module','generics','nullable');
    this.mapped[85] = abi.type_by_fqn.get(debug.FnMode._type)!;
    this.mapped[85].resolveGeneratedOffsetWithValues('read_only', null,'copy_on_write', null,'volatile', null,'write', null);
    this.mapped[86] = abi.type_by_fqn.get(debug.TypeAttr._type)!;
    this.mapped[86].resolveGeneratedOffsets('name','type','is_static');
    this.mapped[87] = abi.type_by_fqn.get(debug.TypeDesc._type)!;
    this.mapped[87].resolveGeneratedOffsets('module','name','methods','attrs','is_abstract','is_enum');
    this.mapped[88] = abi.type_by_fqn.get(debug.FnDesc._type)!;
    this.mapped[88].resolveGeneratedOffsets('module','name','parent_type','params','return_type','is_static','is_task','is_exclusive','is_reserved','is_exposed','mode','permissions');
    this.mapped[89] = abi.type_by_fqn.get(debug.ModVarDesc._type)!;
    this.mapped[89].resolveGeneratedOffsets('module','name','type');
    this.mapped[90] = abi.type_by_fqn.get(debug.FnParam._type)!;
    this.mapped[90].resolveGeneratedOffsets('name','type');
    this.mapped[91] = abi.type_by_fqn.get(math.MathConstants._type)!;
    this.mapped[91].static_values = [2.7182818285,1.4426950409,0.4342944819,0.6931471806,2.302585093,3.1415926536,1.5707963268,0.7853981634,0.3183098862,0.6366197724,1.1283791671,1.4142135624,0.7071067812];
    this.mapped[92] = abi.type_by_fqn.get(util.ByteArray._type)!;
    this.mapped[93] = abi.type_by_fqn.get(util.Iban._type)!;
    this.mapped[94] = abi.type_by_fqn.get(util.TimeWindow._type)!;
    ...
    this.mapped[96] = abi.type_by_fqn.get(util.BoxPlot._type)!;
    ...
    this.mapped[97] = abi.type_by_fqn.get(util.Quantizer._type)!;
    this.mapped[98] = abi.type_by_fqn.get(util.HistogramF64._type)!;
    this.mapped[99] = abi.type_by_fqn.get(util.Random._type)!;
    this.mapped[99].resolveGeneratedOffsets('seed','v');
    this.mapped[100] = abi.type_by_fqn.get(util.NdEncoding._type)!;
    this.mapped[101] = abi.type_by_fqn.get(util.Crypto._type)!;
    this.mapped[102] = abi.type_by_fqn.get(util.HistogramI64._type)!;
    this.mapped[103] = abi.type_by_fqn.get(util.ProgressTracker._type)!;
    this.mapped[104] = abi.type_by_fqn.get(util.Gaussian._type)!;
    this.mapped[105] = abi.type_by_fqn.get(util.GaussianProfile._type)!;
    this.mapped[106] = abi.type_by_fqn.get(util.SlidingWindow._type)!;
    this.mapped[107] = abi.type_by_fqn.get(util.Assert._type)!;
    this.mapped[108] = abi.type_by_fqn.get(util.Histogram._type)!;
    this.mapped[109] = abi.type_by_fqn.get(util.BoxPlotF64._type)!;
    this.mapped[109].resolveGeneratedOffsets('min','max','whiskerLow','whiskerHigh','percentile1','percentile5','percentile25','percentile50','percentile75','percentile95','percentile99','countOutliersLow','countOutliersHigh','percentageOutliersLow','percentageOutliersHigh','sum','avg','std','size');
    this.mapped[110] = abi.type_by_fqn.get(util.Queue._type)!;
    this.mapped[111] = abi.type_by_fqn.get(util.Buffer._type)!;
    this.mapped[112] = abi.type_by_fqn.get(util.HistogramBucket._type)!;
    this.mapped[112].resolveGeneratedOffsetWithValues('from', 0,'to', 1,'count', 2,'percentage', 3);
  },
};
