...
import { core, util, runtime, math, io, debug, } from './lib/index.js';
...
export const stdlib: Library = {
  ...
  init(abi) {
    this.mapped.length = 110;
    this.mapped[0] = abi.type_by_fqn.get(core.String._type)!;
    this.mapped[1] = abi.type_by_fqn.get(core.GeoBox._type)!;
    ...
    this.mapped[6] = abi.type_by_fqn.get(core.nodeTimeCursor._type)!;
    this.mapped[7] = abi.type_by_fqn.get(core.TableColumnMeta._type)!;
    ...
    this.mapped[15] = abi.type_by_fqn.get(core.TensorType._type)!;
    ...
    this.mapped[22] = abi.type_by_fqn.get(core.Map._type)!;
    this.mapped[23] = abi.type_by_fqn.get(core.duration._type)!;
    this.mapped[24] = abi.type_by_fqn.get(core.ti3d._type)!;
    this.mapped[25] = abi.type_by_fqn.get(core.nodeTime._type)!;
    this.mapped[26] = abi.type_by_fqn.get(core.Array._type)!;
    ...
    this.mapped[31] = abi.type_by_fqn.get(core.nodeIndex._type)!;
    ...
    this.mapped[34].static_values = [abi.createTime(-9223372036854775808n),abi.createTime(9223372036854775807n)];
    this.mapped[35] = abi.type_by_fqn.get(core.DatePart._type)!;
    this.mapped[35].resolveGeneratedOffsetWithValues('years', 0,'months', 1,'days', 2,'hours', 3,'minutes', 4,'seconds', 5,'microseconds', 6);
    this.mapped[36] = abi.type_by_fqn.get(core.nodeGeo._type)!;
    this.mapped[37] = abi.type_by_fqn.get(core.ti5d._type)!;
    this.mapped[38] = abi.type_by_fqn.get(core.tf4d._type)!;
    this.mapped[39] = abi.type_by_fqn.get(util.HistogramF64._type)!;
    this.mapped[40] = abi.type_by_fqn.get(util.Iban._type)!;
    this.mapped[41] = abi.type_by_fqn.get(util.BoxPlotI64._type)!;
    this.mapped[41].resolveGeneratedOffsets('min','max','whiskerLow','whiskerHigh','percentile1','percentile5','percentile25','percentile50','percentile75','percentile95','percentile99','countOutliersLow','countOutliersHigh','percentageOutliersLow','percentageOutliersHigh','sum','avg','std','size');
    this.mapped[42] = abi.type_by_fqn.get(util.NdEncoding._type)!;
    this.mapped[43] = abi.type_by_fqn.get(util.Queue._type)!;
    this.mapped[44] = abi.type_by_fqn.get(util.Assert._type)!;
    this.mapped[45] = abi.type_by_fqn.get(util.Quantizer._type)!;
    this.mapped[46] = abi.type_by_fqn.get(util.Crypto._type)!;
    this.mapped[47] = abi.type_by_fqn.get(util.ProgressTracker._type)!;
    this.mapped[48] = abi.type_by_fqn.get(util.BoxPlot._type)!;
    this.mapped[48].resolveGeneratedOffsets('min','max','whiskerLow','whiskerHigh','percentile1','percentile5','percentile25','percentile50','percentile75','percentile95','percentile99','countOutliersLow','countOutliersHigh','percentageOutliersLow','percentageOutliersHigh','sum','avg','std','size');
    this.mapped[49] = abi.type_by_fqn.get(util.BoxPlotF64._type)!;
    ...
    this.mapped[50] = abi.type_by_fqn.get(util.Gaussian._type)!;
    this.mapped[51] = abi.type_by_fqn.get(util.Buffer._type)!;
    this.mapped[52] = abi.type_by_fqn.get(util.ByteArray._type)!;
    this.mapped[53] = abi.type_by_fqn.get(util.TimeWindow._type)!;
    this.mapped[54] = abi.type_by_fqn.get(util.HistogramBucket._type)!;
    this.mapped[54].resolveGeneratedOffsetWithValues('from', 0,'to', 1,'count', 2,'percentage', 3);
    this.mapped[55] = abi.type_by_fqn.get(util.Random._type)!;
    this.mapped[55].resolveGeneratedOffsets('seed','v');
    this.mapped[56] = abi.type_by_fqn.get(util.GaussianProfile._type)!;
    this.mapped[57] = abi.type_by_fqn.get(util.SlidingWindow._type)!;
    this.mapped[58] = abi.type_by_fqn.get(util.Histogram._type)!;
    this.mapped[59] = abi.type_by_fqn.get(util.HistogramI64._type)!;
    this.mapped[60] = abi.type_by_fqn.get(runtime.StoreStat._type)!;
    this.mapped[60].resolveGeneratedOffsets('capacity_bytes','allocated_bytes','allocated_ratio','remained_bytes','remained_ratio','used_bytes','used_ratio','available_bytes','available_ratio');
    this.mapped[61] = abi.type_by_fqn.get(runtime.UserGroupPolicy._type)!;
    this.mapped[61].resolveGeneratedOffsets('group_id','type');
    this.mapped[62] = abi.type_by_fqn.get(runtime.OpenIDConnect._type)!;
    this.mapped[62].resolveGeneratedOffsets('url','clientId');
    this.mapped[63] = abi.type_by_fqn.get(runtime.UserGroup._type)!;
    this.mapped[63].resolveGeneratedOffsets('id','name','activated');
    this.mapped[64] = abi.type_by_fqn.get(runtime.Task._type)!;
    this.mapped[64].resolveGeneratedOffsets('id','user','parent','mod','type','fun','progress','remaining','creation','start','duration','status','sub_waiting','sub_tasks_all');
    this.mapped[65] = abi.type_by_fqn.get(runtime.UserCredential._type)!;
    this.mapped[65].resolveGeneratedOffsets('offset','pass');
    this.mapped[66] = abi.type_by_fqn.get(runtime.User._type)!;
    this.mapped[66].resolveGeneratedOffsets('id','name','activated','full_name','email','role','permissions_flags','groups','groups_flags','external');
    this.mapped[67] = abi.type_by_fqn.get(runtime.SecurityPolicy._type)!;
    this.mapped[67].resolveGeneratedOffsets('entities','credentials','roles','fields');
    this.mapped[68] = abi.type_by_fqn.get(runtime.UserRole._type)!;
    this.mapped[68].resolveGeneratedOffsets('name','permissions');
    ...
    this.mapped[71] = abi.type_by_fqn.get(runtime.SecurityFields._type)!;
    this.mapped[71].resolveGeneratedOffsets('email','name','first_name','last_name','roles','groups');
    this.mapped[72] = abi.type_by_fqn.get(runtime.UserGroupPolicyType._type)!;
    this.mapped[72].resolveGeneratedOffsetWithValues('read', 0,'write', 1,'execute', 2);
    this.mapped[73] = abi.type_by_fqn.get(runtime.System._type)!;
    this.mapped[74] = abi.type_by_fqn.get(runtime.PeriodicTask._type)!;
    this.mapped[74].resolveGeneratedOffsets('name','user_id','args','start','every');
    this.mapped[75] = abi.type_by_fqn.get(runtime.SecurityEntity._type)!;
    this.mapped[75].resolveGeneratedOffsets('id','name','activated');
    this.mapped[76] = abi.type_by_fqn.get(runtime.RuntimeInfo._type)!;
    this.mapped[76].resolveGeneratedOffsets('version','arch','timezone','license','io_threads','bg_threads','fg_threads','mem_total','mem_worker','nb_ctx','store_stats');
    ...
    this.mapped[81].resolveGeneratedOffsets('name','mandatory','trim','try_number','try_json','values','encoder');
    this.mapped[82] = abi.type_by_fqn.get(io.JsonFormat._type)!;
    this.mapped[83] = abi.type_by_fqn.get(io.Env._type)!;
    this.mapped[84] = abi.type_by_fqn.get(io.File._type)!;
    this.mapped[85] = abi.type_by_fqn.get(io.CSVColumnTime._type)!;
    this.mapped[85].resolveGeneratedOffsets('name','mandatory','unit');
    this.mapped[86] = abi.type_by_fqn.get(io.CSVColumnFloat._type)!;
    this.mapped[86].resolveGeneratedOffsets('name','mandatory');
    ...
    this.mapped[88] = abi.type_by_fqn.get(io.TextFormat._type)!;
    this.mapped[89] = abi.type_by_fqn.get(io.JSON._type)!;
    this.mapped[90] = abi.type_by_fqn.get(io.CSVColumnDate._type)!;
    this.mapped[90].resolveGeneratedOffsets('name','mandatory','format','tz','as_time');
    this.mapped[91] = abi.type_by_fqn.get(io.CSVColumnIgnored._type)!;
    this.mapped[91].resolveGeneratedOffsets('name','mandatory');
    this.mapped[92] = abi.type_by_fqn.get(io.FileWriter._type)!;
    this.mapped[93] = abi.type_by_fqn.get(io.FileDescriptor._type)!;
    this.mapped[93].resolveGeneratedOffsets('uri','size','last_modification');
    this.mapped[94] = abi.type_by_fqn.get(io.BinaryFormat._type)!;
    this.mapped[95] = abi.type_by_fqn.get(io.CSVColumn._type)!;
    this.mapped[95].resolveGeneratedOffsets('name','mandatory');
    this.mapped[96] = abi.type_by_fqn.get(io.CSVFormat._type)!;
    this.mapped[96].resolveGeneratedOffsets('header_lines','infer','separator','string_delimiter','decimal_separator','thousands_separator','columns');
    this.mapped[97] = abi.type_by_fqn.get(io.CSVColumnBoolean._type)!;
    this.mapped[97].resolveGeneratedOffsets('name','mandatory');
    this.mapped[98] = abi.type_by_fqn.get(io.AbiWriter._type)!;
    this.mapped[99] = abi.type_by_fqn.get(io.FileFormat._type)!;
    this.mapped[100] = abi.type_by_fqn.get(io.Directory._type)!;
    this.mapped[101] = abi.type_by_fqn.get(io.CSVColumnInteger._type)!;
    this.mapped[101].resolveGeneratedOffsets('name','mandatory');
    this.mapped[102] = abi.type_by_fqn.get(debug.ModDesc._type)!;
    this.mapped[102].resolveGeneratedOffsets('name','lib','functions','types','vars');
    this.mapped[103] = abi.type_by_fqn.get(debug.TypeDesc._type)!;
    this.mapped[103].resolveGeneratedOffsets('module','name','methods','attrs','is_abstract','is_enum');
    this.mapped[104] = abi.type_by_fqn.get(debug.FnMode._type)!;
    this.mapped[104].resolveGeneratedOffsetWithValues('read_only', null,'copy_on_write', null,'volatile', null,'write', null);
    this.mapped[105] = abi.type_by_fqn.get(debug.FnParam._type)!;
    this.mapped[105].resolveGeneratedOffsets('name','type');
    this.mapped[106] = abi.type_by_fqn.get(debug.FnDesc._type)!;
    this.mapped[106].resolveGeneratedOffsets('module','name','parent_type','params','return_type','is_static','is_task','is_exclusive','is_reserved','is_exposed','mode','permissions');
    this.mapped[107] = abi.type_by_fqn.get(debug.TypeAttr._type)!;
    this.mapped[107].resolveGeneratedOffsets('name','type','is_static');
    this.mapped[108] = abi.type_by_fqn.get(debug.TypeRef._type)!;
    this.mapped[108].resolveGeneratedOffsets('name','module','generics','nullable');
    this.mapped[109] = abi.type_by_fqn.get(debug.ModVarDesc._type)!;
    this.mapped[109].resolveGeneratedOffsets('module','name','type');
  },
};
